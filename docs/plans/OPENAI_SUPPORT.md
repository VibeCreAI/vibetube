...
**Issue:** [#10 OpenAI API compatibility](https://github.com/jamiepine/VibeTube/issues/10)

## Overview

This feature exposes OpenAI-compatible endpoints from VibeTube, allowing any tool, library, or application that speaks the OpenAI Audio API to use VibeTube as a drop-in local replacement.
...
flowchart LR
    ...
    subgraph VibeTube [VibeTube Server]
...
- **OpenAI SDK users**: `openai.audio.speech.create()` works with VibeTube
- **LLM frameworks**: LangChain, AutoGen, etc. can use VibeTube for TTS
...
- `voice` parameter maps to VibeTube profile names (case-insensitive)
...
async def resolve_voice_for_openai(voice: str, db: Session) -> Optional[VoiceProfile]:
    """
    Resolve OpenAI voice parameter to a VibeTube profile.
