...
description: "How the multi-voice timeline editor works in VibeTube"
---
...
| POST | `/stories/batch` | Create a new story from multiple generated rows |
| POST | `/stories/import-json` | Import a JSON script and create a new story |
...
Story mode also supports creating a new story from multiple generated rows in one request.
...
1. All `profile_name` values are validated before any generation begins.
2. Entries are generated sequentially.
3. If any entry fails, the batch is rolled back and no partial story remains.
4. If `auto_render` is enabled, the new story is passed into the existing VibeTube story render path.

### JSON Import

`POST /stories/import-json`

Upload a `.json` file containing the same schema as the batch request above.
