...
description: "How text-to-speech generation works in VibeTube"
---

## Overview

VibeTube uses Qwen3-TTS for voice cloning and text-to-speech generation. The TTS module handles model loading, voice prompt creation, and audio synthesis.
...
When switching between model sizes (1.7B â†” 0.6B):
...
1. **Request** â†’ Validate text and profile ID
2. **Profile** â†’ Load profile samples from database
3. **Voice Prompt** â†’ Create or retrieve cached prompt
4. **Generate** â†’ Run TTS inference
5. **Save** â†’ Write audio to generations directory
6. **Record** â†’ Create history entry in database
7. **Response** â†’ Return audio path and metadata
