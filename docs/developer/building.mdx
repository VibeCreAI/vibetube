---
title: "Building"
description: "Build Vibetube for production"
---

## Overview

Vibetube uses a multi-step build process to create platform-specific installers.
...
- macOS (Apple Silicon): `vibetube-server-aarch64-apple-darwin` (includes MLX backend)
- macOS (Intel): `vibetube-server-x86_64-apple-darwin` (PyTorch backend)
- Windows: `vibetube-server-x86_64-pc-windows-msvc.exe` (PyTorch backend)
- Linux: `vibetube-server-x86_64-unknown-linux-gnu` (PyTorch backend)
...
See the [repository](https://github.com/jamiepine/vibetube) for the full CI/CD configuration.
