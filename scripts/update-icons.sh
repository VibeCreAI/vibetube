...
EXPORTS_DIR="tauri/assets/vibetube_exports"
ICON_BUNDLE="tauri/assets/vibetube.icon"
ASSETS_DIR="$ICON_BUNDLE/Assets"
ICONS_DIR="tauri/src-tauri/icons"
LANDING_LOGO="landing/public/vibetube-logo.png"
LANDING_PUBLIC="landing/public"
SOURCE_ICON="$EXPORTS_DIR/vibetube-iOS-Dark-1024x1024@1x.png"

echo "🎨 Updating all Vibetube icons from exports..."
echo ""
...
echo "Compiling vibetube.icon with actool..."
...
if [ -f "tauri/src-tauri/gen/vibetube.icns" ]; then
  echo "  ✓ vibetube.icns generated"
else
  echo "  ⚠ Warning: vibetube.icns not generated (will use fallback)"
fi
...
if [ -f "tauri/src-tauri/gen/vibetube.icns" ]; then
  cp tauri/src-tauri/gen/vibetube.icns "$ICONS_DIR/icon.icns"
  echo "  ✓ Copied Liquid Glass compiled icon.icns"
else
  echo "  ⚠ Liquid Glass icon not found, generating fallback icon.icns..."
mkdir -p /tmp/vibetube-iconset.iconset
sips -s format png -z 16 16 "$SOURCE_ICON" --out /tmp/vibetube-iconset.iconset/icon_16x16.png 2>/dev/null
sips -s format png -z 32 32 "$SOURCE_ICON" --out /tmp/vibetube-iconset.iconset/icon_16x16@2x.png 2>/dev/null
sips -s format png -z 32 32 "$SOURCE_ICON" --out /tmp/vibetube-iconset.iconset/icon_32x32.png 2>/dev/null
sips -s format png -z 64 64 "$SOURCE_ICON" --out /tmp/vibetube-iconset.iconset/icon_32x32@2x.png 2>/dev/null
sips -s format png -z 128 128 "$SOURCE_ICON" --out /tmp/vibetube-iconset.iconset/icon_128x128.png 2>/dev/null
sips -s format png -z 256 256 "$SOURCE_ICON" --out /tmp/vibetube-iconset.iconset/icon_128x128@2x.png 2>/dev/null
sips -s format png -z 256 256 "$SOURCE_ICON" --out /tmp/vibetube-iconset.iconset/icon_256x256.png 2>/dev/null
sips -s format png -z 512 512 "$SOURCE_ICON" --out /tmp/vibetube-iconset.iconset/icon_256x256@2x.png 2>/dev/null
sips -s format png -z 512 512 "$SOURCE_ICON" --out /tmp/vibetube-iconset.iconset/icon_512x512.png 2>/dev/null
  sips -s format png -z 1024 1024 "$SOURCE_ICON" --out /tmp/vibetube-iconset.iconset/icon_512x512@2x.png 2>/dev/null
  iconutil -c icns /tmp/vibetube-iconset.iconset -o "$ICONS_DIR/icon.icns"
  rm -rf /tmp/vibetube-iconset.iconset
